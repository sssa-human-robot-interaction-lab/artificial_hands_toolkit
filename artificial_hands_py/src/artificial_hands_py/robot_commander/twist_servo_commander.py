...
class TwistServoCommander(ServoCommanderBase):
  ...
  def servo_delta(self, goal_time : float, type : str = 'harmonic', delta_pos : Point = Point(), delta_rot : Point = Point()):

    def harmonic_delta(goal_time : float, dt : float, delta_pos : Point = Point(), delta_rot : Point = Point()):
      ...
      servo_wx = np.ndarray.tolist(harmonic_vel(delta_rot.x,goal_time))
      servo_wy = np.ndarray.tolist(harmonic_vel(delta_rot.y,goal_time))
      servo_wz = np.ndarray.tolist(harmonic_vel(delta_rot.z,goal_time))

      return servo_vx,servo_vy,servo_vz,servo_wx,servo_wy,servo_wz

    def biharmonic_delta_2(goal_time : float, dt : float, delta_pos : Point = Point(), delta_rot : Point = Point()):
      ...
      servo_wx = np.ndarray.tolist(biharmonic_vel_2(delta_rot.x,goal_time))
      servo_wy = np.ndarray.tolist(biharmonic_vel_2(delta_rot.y,goal_time))
      servo_wz = np.ndarray.tolist(biharmonic_vel_2(delta_rot.z,goal_time))

      return servo_vx,servo_vy,servo_vz,servo_wx,servo_wy,servo_wz
    
    def trapezoidal_delta_2(goal_time : float, dt : float, delta_pos : Point = Point(), delta_rot : Point = Point()):
      ...
      servo_wx = np.ndarray.tolist(trapezoidal_vel_2(delta_rot.x,goal_time))
      servo_wy = np.ndarray.tolist(trapezoidal_vel_2(delta_rot.y,goal_time))
      servo_wz = np.ndarray.tolist(trapezoidal_vel_2(delta_rot.z,goal_time))

      return servo_vx,servo_vy,servo_vz,servo_wx,servo_wy,servo_wz

    if type == 'harmonic':
      servo_vx,servo_vy,servo_vz,servo_wx,servo_wy,servo_wz = harmonic_delta(goal_time,self.dt,delta_pos,delta_rot)
    elif type == 'biharmonic2':
      servo_vx,servo_vy,servo_vz,servo_wx,servo_wy,servo_wz = biharmonic_delta_2(goal_time,self.dt,delta_pos,delta_rot)
    elif type == 'trapezoidal':
      servo_vx,servo_vy,servo_vz,servo_wx,servo_wy,servo_wz = trapezoidal_delta_2(goal_time,self.dt,delta_pos,delta_rot)
