...
#define SPEED			500		/* 1000 / SPEED = Speed in Hz */
...
{
	w->wrench.force.x = r.fx / FORCE_DIV;
	w->wrench.force.y = r.fy / FORCE_DIV;
	w->wrench.force.z = r.fz / FORCE_DIV;
	w->wrench.torque.x = r.tx / TORQUE_DIV;
	w->wrench.torque.y = r.ty / TORQUE_DIV;
	w->wrench.torque.z = r.tz / TORQUE_DIV;
}

int main ( int argc, char ** argv ) 
{
  ros::init(argc,argv,"onrobot_hex_driver_node");
  ros::NodeHandle nh;
  ros::Publisher pub = nh.advertise<geometry_msgs::WrenchStamped>("/ethdaq_data",1000);
  geometry_msgs::WrenchStamped msg;
  msg.header.frame_id = "hex_sensor_frame";
