...
namespace atk{
  ...
  class Filter
  {
    public:
      /**
       * @brief Construct a new Filter object (simple moving average)
      */
      Filter(){};
  ...
  class IRFilter
  {
    public:
      ...
      /**
       * @brief Construct a new IRFilter object (coeffients based)
      */
      IRFilter(){};
       ...
       */
      IRFilter(double* num, double* den, int order, int chann):
        num_(num),
        den_(den),
        order_(order),
        chann_(chann)
